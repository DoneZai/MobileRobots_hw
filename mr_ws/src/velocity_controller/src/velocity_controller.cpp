...
void on_command_velocity(const std_msgs::Float32& msg) {
  ...
  err.data = desired_velocity - filtered_current_velocity; // change the error to error between the desired and the filtered 
...
void on_timer(const ros::TimerEvent& event) {
  ...
  double stddev = 10;
  double filtered_current_velocity = gaussian_filter(velocity_history, velocity_history.size()-1, window_size, stddev);
