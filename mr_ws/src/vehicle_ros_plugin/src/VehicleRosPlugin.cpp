...
void VehicleRosPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  gazebo_ros->getParameter<std::string>(
    steering_angle_topic, "steering_angle_topic", "steering_angle");
  ...
  joints[FRONT_LEFT_WHEEL] =
    gazebo_ros->getJoint(parent, "front_left_wheel_joint", "front_left_wheel_joint");
  joints[FRONT_RIGHT_WHEEL] =
    gazebo_ros->getJoint(parent, "front_right_wheel_joint", "front_right_wheel_joint");
  joints[REAR_LEFT_WHEEL] =
    gazebo_ros->getJoint(parent, "rear_left_wheel_joint", "rear_left_wheel_joint");
  joints[REAR_RIGHT_WHEEL] =
    gazebo_ros->getJoint(parent, "rear_right_wheel_joint", "rear_right_wheel_joint");
  joints[FRONT_LEFT_STEERING] =
    gazebo_ros->getJoint(parent, "front_left_steering_joint", "front_left_steering_joint");
  joints[FRONT_RIGHT_STEERING] =
    gazebo_ros->getJoint(parent, "front_right_steering_joint", "front_right_steering_joint");
  ...
  cmd_vel_sub = gazebo_ros->node()->subscribe<geometry_msgs::Twist>(
    command_topic, 1, &VehicleRosPlugin::cmdVelCallback, this);

  steering_angle_sub = gazebo_ros->node()->subscribe<std_msgs::Float32>(
    steering_angle_topic, 1, &VehicleRosPlugin::steeringAngleCallback, this);

  velocity_sub = gazebo_ros->node()->subscribe<std_msgs::Float32>(
    velocity_topic, 1, &VehicleRosPlugin::velocityCallback, this);
  ros_queue_thread = std::thread(std::bind(&VehicleRosPlugin::QueueThread, this));

  update_connection =
    event::Events::ConnectWorldUpdateBegin(std::bind(&VehicleRosPlugin::UpdateChild, this));
...
void VehicleRosPlugin::UpdateChild()
{
  ...
  if (seconds_since_last_udate > update_period) {
    ...
    publishOdometry();
    if (this->pulishJointState) publishWheelJointStates();
...
#endif
    //    ROS_INFO_STREAM("set front wheel rate "<<cmd_rate[FRONT_LEFT_WHEEL]<<"
    //    "<<cmd_rate[FRONT_RIGHT_WHEEL]);
  }
  if (drive_wheels == Wheels::REAR || drive_wheels == Wheels::ALL) {
#if GAZEBO_MAJOR_VERSION > 2
...
#endif
    //    ROS_INFO_STREAM("set rear wheel rate "<<cmd_rate[REAR_LEFT_WHEEL]<<"
    //    "<<cmd_rate[REAR_RIGHT_WHEEL]);
  }
}
